...
TEST_CASE("Multiplying by a translation matrix") {
    ...
    REQUIRE_TUPLES(transform * p, point (2,1,7));
}
...
TEST_CASE("Rotating around the x axis") {
    ...
    REQUIRE_TUPLES(halfQuarter * p, point(0,std::sqrt(2)/2,std::sqrt(2)/2));
    REQUIRE_TUPLES(fullQuarter * p, point(0,0,1));
}
...
TEST_CASE("The inverse of the x-rotation matrix rotates in the opposite direction") {
    ...
    REQUIRE_TUPLES(inv * p, point(0,std::sqrt(2)/2,-std::sqrt(2)/2));
}
...
TEST_CASE("Rotating a point around the y axis") {
    ...
    REQUIRE_TUPLES(halfQuarter * p, point(std::sqrt(2) / 2, 0, std::sqrt(2) / 2));
    REQUIRE_TUPLES(fullQuarter * p, point(1, 0, 0));
}
...
TEST_CASE("Rotating a point around the z axis") {
    ...
    REQUIRE_TUPLES(halfQuarter * p, point(-std::sqrt(2) / 2, std::sqrt(2) / 2, 0));
    REQUIRE_TUPLES(fullQuarter * p, point(-1, 0, 0));
}
...
TEST_CASE("Individual transformations are applied in sequence") {
    ...
    const auto p2 = A * p;
    REQUIRE_TUPLES(p2, point(1, -1, 0));

    // Then apply scaling
    const auto p3 = B * p2;
    REQUIRE_TUPLES(p3, point(5, -5, 0));

    // Then apply translation
    const auto p4 = C * p3;
    REQUIRE_TUPLES(p4, point(15, 0, 7));
}
...
TEST_CASE("Chained transformations must be applied in reverse order") {
    ...
    const auto T = C * B * A;
    REQUIRE_TUPLES(T * p, point(15, 0, 7));
}
...
//     REQUIRE_TUPLES(t * point(1, 0, 0), point(1, 0, 0));  // Ensure it's the identity matrix
// }
//
...
//
//     const auto t = viewTransform(from, to, up);
//     REQUIRE_TUPLES(t * point(1, 0, 0), point(-1, 0, 0));  // Ensure it's scaling(-1, 1, -1)
// }
//
...
//
//     const auto t = viewTransform(from, to, up);
//     REQUIRE_TUPLES(t * point(0, 0, 0), point(0, 0, -8));  // Ensure it's translation(0, 0, -8)
// }
//
